...
*ROMA (Ray-aligned Occupancy Map Array)* is a ray tracing alternative, which is fast to build and fast to trace:

- building ROMA only requires one pass of rasterization.
- tracing ray against ROMA only takes O(1) time, without any hierarchical traversal (as opposed to Hardware Ray Tracing) and without iterations (as opposed to Distance Fields)

## Why do we need ROMA in 2023?
